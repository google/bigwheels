...
void FluidSimulation::FreeComputeShaders()
{
...
void FluidSimulation::FreeGraphicShaders()
{
    // Wait for any command buffers in-flight before freeing up resources.
    GetApp()->GetDevice()->WaitIdle();
...
void FluidSimulation::InitGraphicShaders()
{
    // Descriptor set layout.  This is shared across all GraphicShader instances.
    ppx::grfx::DescriptorSetLayoutCreateInfo lci = {};
    lci.bindings.push_back(ppx::grfx::DescriptorBinding(0, ppx::grfx::DESCRIPTOR_TYPE_UNIFORM_BUFFER));
    lci.bindings.push_back(ppx::grfx::DescriptorBinding(1, ppx::grfx::DESCRIPTOR_TYPE_SAMPLED_IMAGE));
    lci.bindings.push_back(ppx::grfx::DescriptorBinding(2, ppx::grfx::DESCRIPTOR_TYPE_SAMPLER));
    ...
    bci.memoryUsage                   = ppx::grfx::MEMORY_USAGE_CPU_TO_GPU;
    PPX_CHECKED_CALL(GetApp()->GetDevice()->CreateBuffer(&bci, &mGraphics.mTransformBuffer));

    ppx::float4x4 mat   = glm::rotate(0.0f, ppx::float3(0, 0, 1));
    void*         pData = nullptr;
    PPX_CHECKED_CALL(mGraphics.mTransformBuffer->MapMemory(0, &pData));
    memcpy(pData, &mat, sizeof(mat));
    mGraphics.mTransformBuffer->UnmapMemory();
    ...
    mDraw = std::make_unique<GraphicShader>(this);
}
...
void FluidSimulation::InitBloomTextures()
{
    ...
    PPX_ASSERT_MSG(mBloomTextures.empty(), "Bloom FBOs already initialized");
