...
#include "PushDescriptorsBuffers.h"
...
void PushDescriptorsBuffersApp::Config(ppx::ApplicationSettings& settings)
{
    settings.appName                    = "25_push_descriptors_buffers";
...
void PushDescriptorsBuffersApp::Setup()
{
    ...
    for (uint32_t i = 0; i < 3; ++i) {
        ...
        PPX_CHECKED_CALL(GetDevice()->CreateBuffer(&createInfo, &mUniformBuffers[i]));
    }
    ...
    // Pipeline
    {
        std::vector<char> bytecode = LoadShader("basic/shaders", "PushDescriptorsBuffersTexture.vs");
        PPX_ASSERT_MSG(!bytecode.empty(), "VS shader bytecode load failed");
        grfx::ShaderModuleCreateInfo shaderCreateInfo = {static_cast<uint32_t>(bytecode.size()), bytecode.data()};
        PPX_CHECKED_CALL(GetDevice()->CreateShaderModule(&shaderCreateInfo, &mVS));

        bytecode = LoadShader("basic/shaders", "PushDescriptorsBuffersTexture.ps");
...
void PushDescriptorsBuffersApp::Render()
{
