...
ppx::Result ForwardRenderer::CreateObjects()
{
    for (uint32_t i = 0; i < GetNumInFlightFrames(); ++i) {
        ...
        PPX_CHECKED_CALL(GetDevice()->GetGraphicsQueue()->CreateCommandBuffer(&frame.outputCmd));
...
void ForwardRenderer::DestroyObjects()
{
    for (auto& frame : mFrames) {
        GetDevice()->GetGraphicsQueue()->DestroyCommandBuffer(frame.outputCmd);
    ...
    grfx::Semaphore*        pRenderCompleteSemaphore)
{
    ...
    //
    ppxres = GetRenderOutputRenderPass(
        pOutputImage,
        &pOutputRenderPass);
    if (Failed(ppxres)) {
        return ppxres;
