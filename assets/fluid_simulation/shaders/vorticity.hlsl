...
[numthreads(1, 1, 1)] void csmain(uint2 tid
    ...
    float L = UCurl.SampleLevel(ClampSampler, coord.vL, 0).x;
    float R = UCurl.SampleLevel(ClampSampler, coord.vR, 0).x;
    float T = UCurl.SampleLevel(ClampSampler, coord.vT, 0).x;
    float B = UCurl.SampleLevel(ClampSampler, coord.vB, 0).x;
    float C = UCurl.SampleLevel(ClampSampler, coord.vUv, 0).x;
