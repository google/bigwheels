...
float4 bilerp(Texture2D sam, float2 vUv, float2 tsize)
{
    ...
    float4 a = sam.SampleLevel(ClampSampler, (iuv + float2(0.5, 0.5)) * tsize, 0);
    float4 b = sam.SampleLevel(ClampSampler, (iuv + float2(1.5, 0.5)) * tsize, 0);
    float4 c = sam.SampleLevel(ClampSampler, (iuv + float2(0.5, 1.5)) * tsize, 0);
    float4 d = sam.SampleLevel(ClampSampler, (iuv + float2(1.5, 1.5)) * tsize, 0);
