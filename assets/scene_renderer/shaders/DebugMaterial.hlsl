...
float4 psmain(StandardVertexOutput input) : SV_TARGET
{
    ...
    if (Draw.dbgVtxAttrIndex == DBG_VTX_ATTR_INDEX_POSITION) {
        color = input.PositionWS.xyz;
    }
    else if(Draw.dbgVtxAttrIndex == DBG_VTX_ATTR_INDEX_TEXCOORD) {
        color = float3(input.TexCoord, 0);
    }
    else if(Draw.dbgVtxAttrIndex == DBG_VTX_ATTR_INDEX_NORMAL) {
